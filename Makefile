...
ifeq ($(SLOW),)
	...
	pytest --exitfirst --splits 5 --group $(PYTEST_GROUP) --store-durations --durations-path=./.test_durations.$(PYTEST_GROUP) --ignore=tests/functional/targets/test_performance.py tests -vvv --cucumberjson=test-report.json --durations=0 --html=test-report.html
else
	echo "Running slow tests"
	pytest --exitfirst tests/functional/targets/test_performance.py -vvv --cucumberjson=test-report.json --durations=0 --html=test-report.html
endif

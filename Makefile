...
ifeq ($(SLOW),)
	...
	pytest --exitfirst --splits 5 --group $(PYTEST_GROUP) --store-durations --durations-path=./.test_durations.$(PYTEST_GROUP) -k "not tests/functional/targets/test_performance.py::test_api_requester_multiple_request_and_targets[50-50-600]" tests -vvv --cucumberjson=test-report.json --durations=0 --html=test-report.html
else

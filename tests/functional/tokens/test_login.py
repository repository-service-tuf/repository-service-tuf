...
@given(
    parse(
        "the API requester prepares 'data' with {username}, {password}, {scope},"
        " {expires}"
    ),
    target_fixture="data",
)
...
@when(
    "the API requester sends a 'POST' method to '/api/v1/token' with invalid username/password in 'data'",
...
@when(
    "the API requester sends a 'POST' method to '/api/v1/token' with invalid scopes in 'data'",
...
@given(
    parse("the user types the command 'rstuf admin login -f -s http://localhost -u {username} -p {password} -e {expires}'"),
    target_fixture="command"
)
def the_user_types_command(username, password, expires):
    return f"admin login -f -s http://localhost -u {username} -p {password} -e {expires}"
...
@scenario("../../features/tokens/login.feature", "Login using RSTUF CLI gets Unauthorized")
def test_login_using_rstuf_cli_gets_unauthorized():
    """Login using RSTUF CLI gets Unauthorized."""


@when(
    "the user 'enter' the login command with invalid username/password credentials",
